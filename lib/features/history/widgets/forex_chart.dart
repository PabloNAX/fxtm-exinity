...
import 'dart:math' show pow, log, sin, max, min;
...
class _ForexChartState extends State<ForexChart> {
  ...
  Widget build(BuildContext context) {
    ...
    double yInterval = yRange / (numberOfYLabels - 1);

    // Round the interval to a nice number
    double roundedInterval = _roundToNiceNumber(yInterval);
    ...
    double visiblePoints = dataLength / _currentScale;
      ...
      child: Padding(
        ...
        child: LineChart(
          LineChartData(
            ...
            minX: minX,
            maxX: maxX,
            clipData: FlClipData.all(), // Ensure chart is clipped to its bounds
          ),
        ),
      ),
    );
  ...
  double _roundToNiceNumber(double value) {
    ...
    num exponent = (value > 0) ? (log(value) / log(10)).floor() : 0;
    double power = pow(10, exponent).toDouble();
